...
const getAdjacentRoomName = function(this: any, direction:ExitConstant):string {
...
const getNumberOfMiningSpacesAtSource = function(this: any, sourceId: Id<Source>) {
...
const getTotalNumberOfMiningSpaces = function(this: any) {
    const sourceMap = this.findNumberOfSourcesAndSpaces();
    if (sourceMap) {
...
const getNumberOfSources = function(this: any) {
    const sourceMap = this.findNumberOfSourcesAndSpaces();
    if (sourceMap) {
...
const findNumberOfSourcesAndSpaces = function(this: any) {
    ...
    _.forEach(this.find(FIND_SOURCES), (source: Source) => {
        ...
        _.forEach(this.lookAtArea(source.pos.y-1, source.pos.x-1, source.pos.y+1, source.pos.x+1, true),
            (lookupObject: LookAtResultWithPos) => {
                if (lookupObject.type === 'structure' && lookupObject.structure &&
...
const getNumberOfCreepsByRole = function(this: any, role: CreepRoleEnum): number {
    ...
    if (creepCountArray === null) {
        ...
        _.forEach(room.find(FIND_MY_CREEPS), (creep: Creep) => {
            if (creep.memory && creep.memory['role']) {
                const currentRole: CreepRoleEnum = creep.memory['role'] as CreepRoleEnum;
...
const reassignAllCreeps = function(this: any, newRole: CreepRoleEnum, filter: Function) {
...
const reassignSingleCreep = function(this: any, newRole: CreepRoleEnum, filter: Function) {
    if (this.creepCountArray == null) {
        this.getNumberOfCreepsByRole(newRole);
    }
...
function canPlaceRampart(pos:RoomPosition):boolean {
    ...
    _.forEach(Game.rooms[pos.roomName].lookAt(pos), (s:LookAtResultWithPos) => {
        if ((s.type === 'structure' && s.structure && s.structure.structureType === STRUCTURE_RAMPART) ||
...
const makeConstructionSites = function(this: any) {
...
const isSpotOpen = function(this: any, pos:RoomPosition):boolean {
...
const reassignIdleCreep = function(this: any, creep: Creep) {
    const oldRole: CreepRoleEnum = creep.memory['role'] as CreepRoleEnum;
...
declare global {
    interface Room {
        ...
        creepCountArray: Map<CreepRoleEnum, number>|null;
